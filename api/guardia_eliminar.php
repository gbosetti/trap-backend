...
if ($registros_como_guardia['count'] > 0 || $registros_como_visitante['count'] > 0) {
    ...
    $sql = "DELETE FROM `usuarios_guardias` WHERE dni_usuario='$dni'";
	if ($mysqli->query($sql) === true) {
	  echo json_encode(array('auth' => true, 'token' => $guard_token, 'error' => false, 'message' => 'El usuario ha sido dado de baja como guardia, sin embargo algunos de sus datos se mantuvieron para no perder la integridad de los movimientos registrados.'));
	} else {
